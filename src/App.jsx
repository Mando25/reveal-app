...
function ChatPanel({ myName, gameId, qIdx }) {
  ...
  useEffect(() => {
    setMessages([]);
    dbGetMsgs(gameId, qIdx).then(setMessages);
    let lastCount = 0;
    const iv = setInterval(async () => {
      const msgs = await dbGetMsgs(gameId, qIdx);
      if (msgs.length > lastCount) { lastCount = msgs.length; setMessages(msgs); }
    }, 1500);
...
function WaitingScreen({ game, myName, onStart, onBack }) {
  ...
  useEffect(() => {
    ...
    const iv = setInterval(async () => { const g=await dbGet(game.id); if(g?.guest_name){setGuestJoined(true);clearInterval(iv);} },2000);
    return ()=>clearInterval(iv);
  },[]);
  return (
    <div className="screen">
...
// ── KEY FIX: always fetch latest from DB before saving answers ──
...
function WaitingRevealScreen({ game, myName, myRole, onReveal }) {
  ...
  useEffect(() => {
    const iv = setInterval(async () => {
      ...
      if (g) {
        setLiveGame(g);
        if (g.reveal_started) { clearInterval(iv); onReveal(g); }
      }
    }, 2000);
    return ()=>clearInterval(iv);
  },[]);
  ...
  return (
    <div className="screen">
      ...
      <div className="card">
        ...
        {otherDone&&myRole==="guest"&&<p style={{fontSize:13,color:"var(--muted)",textAlign:"center",marginTop:10}}>En attente que l'hôte lance le reveal…</p>}
      </div>
    </div>
  );
}
...
function RevealScreen({ game, myRole, myName, revealIdx, onNext, isLast }) {
  ...
  useEffect(() => {
    const iv = setInterval(async()=>{const g=await dbGet(game.id);if(g)setLiveGame(g);},2000);
    return ()=>clearInterval(iv);
  },[]);
...
export default function App() {
  ...
  subscribe(table, gameId, cb) {
    // Polling every 2s for realtime (no WS needed)
    const iv = setInterval(async () => {
      const data = await db.get(table, gameId);
      if (data) cb(data);
    }, 2000);
    return () => clearInterval(iv);
  },
  subscribeMessages(gameId, qIdx, cb) {
    let lastCount = 0;
    const iv = setInterval(async () => {
      const msgs = await db.getMessages(gameId, qIdx);
...
const QUESTIONS = {
  decouverte: {
    classiques: [
      "C'est quoi ton plus grand rêve dans la vie ?",
      "T'as peur de quoi en ce moment ?",
      "Quel est ton meilleur souvenir d'enfance ?",
      "Si tu pouvais changer une chose dans ta vie, ce serait quoi ?",
      "C'est quoi la chose dont tu es le plus fier(e) ?",
      "T'as déjà regretté quelque chose de vraiment important ?",
      "C'est quoi ton endroit préféré dans le monde ?",
      "Si tu pouvais voyager n'importe où demain, tu irais où ?",
      "Qu'est-ce qui te rend heureux(se) instantanément ?",
      "C'est quoi ta plus grande qualité selon toi ?",
      "Et ton plus grand défaut ?",
      "T'as une passion cachée que peu de gens connaissent ?",
      "Si tu devais décrire ta vie en un film, lequel ce serait ?",
      "C'est quoi la dernière chose qui t'a vraiment ému(e) ?",
      "T'aurais fait quoi si tu n'avais pas choisi ta voie actuelle ?",
      "Qu'est-ce que tu aimerais que les gens retiennent de toi ?",
      "C'est quoi ta définition du bonheur ?",
      "T'as une croyance que peu de gens partagent ?",
      "Si tu pouvais dîner avec n'importe qui dans l'histoire, ce serait qui ?",
      "C'est quoi la dernière fois que t'as vraiment ri aux éclats ?",
      "T'as un rituel quotidien auquel tu tiens vraiment ?",
      "C'est quoi ton rapport à la solitude ?",
      "Si tu avais une superpower, ce serait laquelle ?",
      "C'est quoi la chose la plus courageuse que t'as jamais faite ?",
      "T'as déjà vécu un moment qui t'a complètement changé(e) ?",
      "C'est quoi ton plus grand regret ?",
      "T'aimes mieux être aimé(e) ou respecté(e) ?",
      "C'est quoi ta vision d'une vie réussie ?",
      "T'as déjà menti pour protéger quelqu'un ?",
      "C'est quoi la chose la plus difficile que t'aies jamais traversée ?",
      "C'est quoi ton rapport à la famille ?",
      "T'as un objectif que tu n'as pas encore atteint ?",
      "T'aimes mieux les matins ou les soirs ?",
      "T'as des ambitions que tu n'oses pas dire à voix haute ?",
      "C'est quoi le truc le plus bizarre que t'aies jamais fait ?",
      "C'est quoi ta vision de l'amitié parfaite ?",
      "C'est quoi le conseil que tu donnerais à ton toi d'il y a 5 ans ?",
      "T'as un souvenir qui te fait sourire à chaque fois ?",
      "C'est quoi ta philosophie de vie en une phrase ?",
      "T'aimes mieux donner ou recevoir ?",
      "T'as déjà fait une chose que personne ne sait ?",
      "C'est quoi le truc qui te manque le plus en ce moment ?",
      "T'aurais plutôt vivre 100 ans normalement ou 50 ans intensément ?",
      "T'as des pensées que tu n'oses jamais dire ?",
      "T'aimes mieux les surprises ou tout planifier ?",
      "T'as déjà été jaloux(se) de quelqu'un ? De quoi ?",
      "C'est quoi ton rapport au changement ?",
      "T'as déjà pleuré devant quelqu'un et regretté de l'avoir fait ?",
      "C'est quoi le truc que t'as du mal à pardonner ?",
    ],
    creatives: [
      "Quelle chanson résume ta vie en ce moment ?",
      "Si ta vie était une série Netflix, quel serait le titre ?",
      "Quelle couleur tu associes à ton humeur cette semaine ?",
      "Si tu étais un plat, t'aurais quel goût ?",
      "Quelle chanson te ferait pleurer si tu l'écoutais maintenant ?",
      "Si tu pouvais envoyer un message à ton futur toi dans 10 ans, t'écrirais quoi ?",
      "T'es plutôt quel temps qu'il fait aujourd'hui et pourquoi ?",
      "Si ta personnalité était un genre musical, ce serait lequel ?",
      "C'est quoi l'objet chez toi qui a le plus de valeur sentimentale ?",
      "Quelle odeur te rappelle immédiatement quelque chose de fort ?",
      "Si tu étais un personnage de film, lequel tu serais et pourquoi ?",
      "C'est quoi le dernier rêve dont tu te souviens ?",
      "Si ta vie avait une bande-son, quel artiste la composerait ?",
      "T'es plutôt lever de soleil ou coucher de soleil ?",
      "Si tu devais te décrire avec un animal, ce serait lequel ?",
      "Quelle chanson tu mettrais si tu devais entrer dans une pièce de façon épique ?",
      "C'est quoi ton film doudou, celui que t'as vu 10 fois ?",
      "T'associes quelle chanson à ton meilleur souvenir ?",
      "Si ta vie était un livre, t'en serais à quel chapitre ?",
      "C'est quoi le plat qui te ramène immédiatement à ton enfance ?",
      "Si tu devais vivre dans l'univers d'une série, ce serait laquelle ?",
      "Quelle chanson t'écoutes quand tu veux te sentir invincible ?",
      "T'es plutôt océan ou montagne ?",
      "C'est quoi ton endroit parfait pour réfléchir ?",
      "Si tu étais une saison, t'aurais laquelle et pourquoi ?",
      "C'est quoi le livre ou film qui t'a le plus marqué(e) ?",
      "T'es plutôt café du matin ou coucher tard le soir ?",
      "Si ta personnalité était une ville, ce serait laquelle ?",
      "Quelle chanson t'écoutes quand tu es triste mais que tu veux l'être encore plus ?",
      "C'est quoi une image mentale qui te rend instantanément heureux(se) ?",
    ],
  },
  epice: [
    "C'est quoi un fantasme que t'as jamais osé dire ?",
    "T'as déjà envoyé un message à la mauvaise personne ? C'était quoi ?",
    "C'est quoi le truc le plus osé que t'aies jamais fait ?",
    "T'as déjà eu le coup de foudre ? Raconte.",
    "C'est quoi ton plus grand turn-on ?",
    "T'as déjà été attiré(e) par quelqu'un que t'aurais pas dû ?",
    "C'est quoi la chose la plus romantique que t'aies jamais faite ?",
    "T'as déjà menti sur tes sentiments ?",
    "T'as déjà eu des sentiments pour deux personnes en même temps ?",
    "T'as déjà écrit une lettre d'amour que t'as jamais envoyée ?",
    "C'est quoi ton plus grand turn-off ?",
    "T'as déjà fait quelque chose d'interdit et adoré ça ?",
    "C'est quoi le message le plus flirty que t'aies jamais envoyé ?",
    "T'as déjà eu une relation secrète ?",
    "C'est quoi la chose la plus coquine que t'aies jamais faite ?",
    "C'est quoi ton fantasme de lieu ?",
    "T'as déjà embrassé quelqu'un par défi et aimé ça ?",
    "C'est quoi la chose la plus douce qu'on t'ait jamais dite ?",
    "T'as déjà utilisé quelqu'un pour oublier quelqu'un d'autre ?",
    "C'est quoi le truc qui te rend fou/folle chez l'autre ?",
    "T'as déjà eu une nuit que tu n'oublieras jamais ?",
    "C'est quoi ton signe qu'on te plaît vraiment ?",
    "C'est quoi la chose la plus courageuse que t'aies faite par amour ?",
    "T'as déjà eu envie de tout plaquer pour quelqu'un ?",
    "T'as déjà été le/la premier(e) à dire 'je t'aime' ?",
    "C'est quoi ta définition d'une nuit parfaite ?",
    "C'est quoi le geste qui te fond à chaque fois ?",
    "T'as déjà gardé un secret lourd dans une relation ?",
    "T'as déjà été obsédé(e) par quelqu'un ?",
    "T'as déjà aimé quelqu'un qui ne t'aimait pas en retour ?",
    "T'as déjà envoyé une photo que tu regrettes ?",
    "C'est quoi le souvenir le plus chaud que t'aies ?",
    "C'est quoi ton rapport à l'intimité physique vs émotionnelle ?",
    "C'est quoi le rituel que t'as pour te sentir séduisant(e) ?",
    "T'as déjà fait une déclaration qui a tout changé ?",
    "C'est quoi la chose la plus intense que t'aies ressentie pour quelqu'un ?",
  ],
};
...
const DILEMMES = {
  soft: [
    ...
    { a: "Ne plus jamais écouter de musique", b: "Ne plus jamais regarder de films" },
    { a: "Être super riche mais seul(e)", b: "Être fauché(e) mais très entouré(e)" },
    { a: "Tout recommencer à 15 ans avec ta mémoire actuelle", b: "Continuer ta vie mais avec 10 ans de plus" },
    { a: "Toujours dire ce que tu penses", b: "Ne jamais pouvoir mentir" },
    { a: "Lire dans les pensées", b: "Être invisible" },
    { a: "Perdre tous tes souvenirs", b: "Ne jamais en créer de nouveaux" },
    { a: "Vivre 100 ans sans passion", b: "Vivre 50 ans intensément" },
    { a: "Être trop honnête", b: "Être trop gentil(le)" },
    { a: "Tout savoir de ton futur", b: "Ne rien savoir du tout" },
    { a: "Avoir beaucoup d'amis superficiels", b: "Avoir 2 vrais amis pour la vie" },
    { a: "Être célèbre mais sans argent", b: "Être riche mais totalement inconnu(e)" },
    { a: "Savoir ce que les autres pensent vraiment de toi", b: "Ne jamais le savoir" },
    { a: "Ne plus jamais utiliser ton téléphone", b: "Ne plus jamais regarder Netflix" },
    { a: "Être trop sensible", b: "Ne rien ressentir" },
    { a: "Pouvoir voler", b: "Pouvoir être invisible" },
    { a: "Toujours arriver en retard", b: "Toujours arriver bien trop tôt" },
    { a: "Avoir un talent extraordinaire que personne ne verra jamais", b: "Être médiocre mais reconnu(e)" },
    { a: "Rater quelque chose d'important par ta faute", b: "Rater la même chose par la faute de quelqu'un d'autre" },
    { a: "Ne jamais avoir froid", b: "Ne jamais avoir chaud" },
  ],
  epice: [
    { a: "Être quitté(e)", b: "Quitter" },
    { a: "Que ton ex soit heureux(se) sans toi", b: "Qu'il/elle soit malheureux(se) sans toi" },
    { a: "Une relation passionnelle et toxique", b: "Une relation stable mais sans étincelle" },
    { a: "Tout savoir sur les ex de l'autre", b: "Que l'autre sache tout sur les tiens" },
    { a: "Être trompé(e) et ne jamais le savoir", b: "Le savoir et ne pas pouvoir en parler" },
    { a: "Une nuit inoubliable sans lendemain", b: "Une relation longue mais sans folie" },
    { a: "Aimer sans être aimé(e) en retour", b: "Être aimé(e) sans vraiment aimer" },
    { a: "Que l'autre lise tous tes messages", b: "Que l'autre lise tous tes rêves" },
    { a: "Être trop jaloux(se)", b: "Ne ressentir aucune jalousie" },
    { a: "Dire 'je t'aime' en premier sans l'entendre en retour", b: "Ne jamais le dire même si tu le penses" },
    { a: "Que l'autre sache exactement ce que tu aimes au lit", b: "Garder une part de mystère pour toujours" },
    { a: "Toujours prendre l'initiative", b: "Ne jamais la prendre" },
    { a: "Être la meilleure expérience que l'autre ait eue", b: "Que l'autre soit la meilleure que t'aies eue" },
    { a: "Séduire facilement tout le monde sans être vraiment aimé(e)", b: "Séduire une seule personne mais être aimé(e) follement" },
    { a: "Que l'autre connaisse tous tes fantasmes", b: "Découvrir tous les siens" },
    { a: "Rester avec quelqu'un que tu aimes mais qui ne te rend pas heureux(se)", b: "Le quitter et risquer de le regretter toute ta vie" },
    { a: "Une passion qui détruit tout", b: "Une tranquillité qui n'excite personne" },
    { a: "Que l'autre change pour toi", b: "Changer pour l'autre" },
    { a: "Une attirance folle sans amour", b: "Un amour profond sans attirance physique" },
    { a: "Que l'autre fantasme sur quelqu'un d'autre", b: "Que tu fantasmes sur quelqu'un d'autre" },
  ],
};

function getQuestions(mode, count) {
  if (mode === "epice") return [...QUESTIONS.epice].sort(() => Math.random() - 0.5).slice(0, count);
  const cl = [...QUESTIONS.decouverte.classiques].sort(() => Math.random() - 0.5);
  const cr = [...QUESTIONS.decouverte.creatives].sort(() => Math.random() - 0.5);
  const nCr = Math.round(count * 0.27);
  return [...cl.slice(0, count - nCr), ...cr.slice(0, nCr)].sort(() => Math.random() - 0.5);
}

function getDilemmes(intensity, count) {
  return [...DILEMMES[intensity]].sort(() => Math.random() - 0.5).slice(0, count);
}
...
const G = `
  ...
  *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
  :root {
    --bg:#0d0d10; --s1:#15151b; --s2:#1e1e27; --s3:#27272f;
    --border:rgba(255,255,255,0.07);
    --gold:#d4a853; --gold2:#f0cc7a;
    --text:#ece8e0; --muted:rgba(236,232,224,0.38); --muted2:rgba(236,232,224,0.65);
    --green:#6fcf8a; --red:#e07878;
  }
  html,body{height:100%;}
  body{background:var(--bg);color:var(--text);font-family:'Instrument Sans',sans-serif;-webkit-font-smoothing:antialiased;}
  ::-webkit-scrollbar{width:3px;} ::-webkit-scrollbar-track{background:transparent;} ::-webkit-scrollbar-thumb{background:var(--s3);border-radius:2px;}
  ...
  @keyframes up{from{opacity:0;transform:translateY(18px)}to{opacity:1;transform:translateY(0)}}
  .card-title{font-family:'Fraunces',serif;font-size:24px;font-weight:600;margin-bottom:22px;letter-spacing:-.5px;}
  .btn{display:flex;align-items:center;justify-content:center;gap:8px;width:100%;padding:15px 20px;border-radius:14px;font-family:'Instrument Sans',sans-serif;font-size:15px;font-weight:600;cursor:pointer;border:none;transition:all .18s;letter-spacing:-.1px;}
  .btn+.btn{margin-top:10px;}
  .btn-gold{background:linear-gradient(135deg,var(--gold),var(--gold2));color:#1a1200;box-shadow:0 4px 20px rgba(212,168,83,.25);}
  .btn-gold:hover:not(:disabled){transform:translateY(-1px);box-shadow:0 6px 24px rgba(212,168,83,.35);}
  .btn-ghost{background:var(--s2);color:var(--muted2);border:1px solid var(--border);}
  .btn-ghost:hover:not(:disabled){border-color:var(--gold);color:var(--text);}
  .btn:disabled{opacity:.35;cursor:not-allowed;}
  .lbl{font-size:11px;color:var(--muted);text-transform:uppercase;letter-spacing:2px;margin-bottom:8px;}
  ...
  .count-row{display:flex;gap:8px;margin-bottom:20px;}
  .cnt-btn{flex:1;padding:11px;border-radius:11px;border:1.5px solid var(--border);background:var(--s2);color:var(--muted2);font-family:'Instrument Sans',sans-serif;font-size:14px;font-weight:600;cursor:pointer;transition:all .18s;}
  .cnt-btn:hover{border-color:rgba(212,168,83,.5);}
  .cnt-btn.sel{border-color:var(--gold);color:var(--gold);background:rgba(212,168,83,.07);}
  .divider{height:1px;background:var(--border);margin:20px 0;}
  .cq-label{font-size:11px;color:var(--muted);text-transform:uppercase;letter-spacing:2px;margin-bottom:10px;}
  .cq-row{display:flex;gap:8px;margin-bottom:10px;}
  .cq-row .inp{margin-bottom:0;flex:1;}
  .cq-add-btn{padding:13px 16px;border-radius:12px;border:1.5px solid var(--gold);background:transparent;color:var(--gold);cursor:pointer;font-size:20px;transition:all .18s;flex-shrink:0;}
  .cq-add-btn:hover{background:rgba(212,168,83,.1);}
  .cq-item{display:flex;align-items:center;gap:8px;background:var(--s2);border:1px solid var(--border);border-radius:11px;padding:10px 14px;font-size:13px;margin-bottom:6px;}
  .cq-item span{flex:1;color:var(--muted2);}
  .cq-rm{background:none;border:none;color:var(--red);cursor:pointer;font-size:17px;line-height:1;}
  .code-box{background:var(--s2);border:1px solid var(--border);border-radius:18px;padding:24px;text-align:center;margin:18px 0;position:relative;overflow:hidden;}
  .code-box::before{content:'';position:absolute;inset:0;background:radial-gradient(ellipse at 50% -20%,rgba(212,168,83,.08),transparent 65%);}
  .code-big{font-family:'Fraunces',serif;font-size:46px;font-weight:600;letter-spacing:10px;color:var(--gold);position:relative;}
  .code-hint{font-size:11px;color:var(--muted);margin-top:6px;letter-spacing:1px;position:relative;}
  .status-row{display:flex;gap:8px;margin:14px 0;}
  .pill{flex:1;padding:11px 12px;border-radius:11px;text-align:center;background:var(--s2);border:1px solid var(--border);font-size:13px;color:var(--muted2);transition:all .3s;}
  .pill.ok{border-color:var(--green);color:var(--green);background:rgba(111,207,138,.06);}
  .dots{display:flex;gap:6px;justify-content:center;margin:14px 0;}
  ...
  .q-screen{min-height:100vh;display:flex;flex-direction:column;background:var(--bg);}
  .q-header{padding:20px 24px 16px;border-bottom:1px solid var(--border);display:flex;align-items:center;justify-content:space-between;background:var(--s1);}
  .q-counter-badge{font-size:12px;font-weight:600;color:var(--gold);background:rgba(212,168,83,.1);border:1px solid rgba(212,168,83,.2);padding:5px 12px;border-radius:20px;letter-spacing:.5px;}
  .q-mode-badge{font-size:11px;color:var(--muted);letter-spacing:1.5px;text-transform:uppercase;}
  .q-body{flex:1;padding:28px 24px 20px;display:flex;flex-direction:column;}
  .q-progress{height:2px;background:var(--s3);border-radius:1px;margin-bottom:28px;overflow:hidden;}
  .q-progress-fill{height:100%;background:linear-gradient(90deg,var(--gold),var(--gold2));border-radius:1px;transition:width .5s cubic-bezier(.22,1,.36,1);}
  ...
  .d-screen{min-height:100vh;display:flex;flex-direction:column;background:var(--bg);}
  .d-body{flex:1;padding:24px 20px;display:flex;flex-direction:column;gap:14px;}
  .d-question{font-family:'Fraunces',serif;font-size:20px;font-weight:300;font-style:italic;line-height:1.5;color:var(--text);text-align:center;padding:8px 0 4px;}
  .d-vs{text-align:center;font-size:11px;color:var(--muted);letter-spacing:3px;text-transform:uppercase;margin:2px 0;}
  .d-choice{flex:1;padding:20px 18px;border-radius:18px;border:1.5px solid var(--border);background:var(--s2);cursor:pointer;transition:all .2s;display:flex;align-items:center;justify-content:center;text-align:center;font-size:15px;font-weight:500;line-height:1.5;color:var(--muted2);min-height:90px;}
  .d-choice:hover{border-color:rgba(212,168,83,.4);color:var(--text);}
  .d-choice.chosen{border-color:var(--gold);background:rgba(212,168,83,.08);color:var(--text);}
  .d-footer{padding:16px 20px 28px;}
  ...
  .rv-counter{font-size:12px;color:var(--muted);}
  .rv-progress{height:2px;background:var(--s3);border-radius:1px;overflow:hidden;}
  .rv-progress-fill{height:100%;background:linear-gradient(90deg,var(--gold),var(--gold2));border-radius:1px;transition:width .5s cubic-bezier(.22,1,.36,1);}
  .rv-question-box{padding:14px 20px;border-bottom:1px solid var(--border);flex-shrink:0;}
  .rv-question-text{font-family:'Fraunces',serif;font-size:16px;font-weight:300;font-style:italic;color:var(--text);line-height:1.5;}
  .rv-answers{display:flex;gap:10px;padding:12px 16px;border-bottom:1px solid var(--border);flex-shrink:0;}
  .rv-answer-card{flex:1;background:var(--s2);border:1px solid var(--border);border-radius:14px;padding:12px 13px;animation:pop .3s cubic-bezier(.22,1,.36,1);}
  ...
  .rv-chat{flex:1;display:flex;flex-direction:column;overflow:hidden;}
  .chat-msgs{flex:1;overflow-y:auto;padding:12px 16px;display:flex;flex-direction:column;gap:5px;}
  .chat-bubble-wrap{display:flex;flex-direction:column;animation:msgIn .2s cubic-bezier(.22,1,.36,1);}
  @keyframes msgIn{from{opacity:0;transform:translateY(5px)}to{opacity:1;transform:translateY(0)}}
  .chat-bubble-wrap.me{align-items:flex-end;}
  .chat-bubble-wrap.them{align-items:flex-start;}
  .chat-sender{font-size:10px;color:var(--muted);margin-bottom:2px;padding:0 6px;letter-spacing:.5px;}
  .chat-bubble{max-width:78%;padding:9px 14px;border-radius:18px;font-size:14px;line-height:1.5;word-break:break-word;}
  .chat-bubble.me{background:var(--gold);color:#1a1200;border-bottom-right-radius:5px;}
  .chat-bubble.them{background:var(--s2);color:var(--text);border:1px solid var(--border);border-bottom-left-radius:5px;}
  .chat-bubble.emoji-only{background:transparent!important;border:none!important;font-size:26px;padding:3px 5px;}
...
function ChatPanel({ myName, gameId, qIdx }) {
  ...
  useEffect(() => {
    setMessages([]);
    db.getMessages(gameId, qIdx).then(setMessages);
    const unsub = db.subscribeMessages(gameId, qIdx, setMessages);
    return unsub;
  }, [gameId, qIdx]);
  ...
  async function send(text) {
    if (!text.trim()) return;
    setInput("");
    await db.insertMessage({ game_id: gameId, sender: myName, text: text.trim(), question_index: qIdx });
  }
...
function CreateScreen({ onStart, onBack }) {
  ...
  const addQ = () => {
    if (cInput.trim() && customQs.length < 2) { setCustomQs([...customQs, cInput.trim()]); setCInput(""); }
  };
  ...
  return (
    <div className="screen" style={{justifyContent:"flex-start",paddingTop:36}}>
      <div style={{textAlign:"center",marginBottom:4}}>
        <div className="wordmark" style={{fontSize:36}}>Reveal</div>
      </div>
      <div className="card">
        <div className="card-title">Nouvelle partie</div>
        <div className="lbl">Ton prénom</div>
        ...
        {!isDilemme && <>
          ...
          {customQs.length < 2 && (
            <div className="cq-row">
              <input className="inp" placeholder="Ta question perso..." value={cInput} onChange={e=>setCInput(e.target.value)} onKeyDown={e=>e.key==="Enter"&&addQ()} />
              <button className="cq-add-btn" onClick={addQ}>+</button>
            </div>
          )}
          {customQs.map((q,i) => (
            <div key={i} className="cq-item">
              <span>{q}</span>
              <button className="cq-rm" onClick={() => setCustomQs(customQs.filter((_,j) => j!==i))}>×</button>
            </div>
          ))}
        </>}
        <button className="btn btn-gold" style={{marginTop:20}} disabled={!mode||!name.trim()||loading} onClick={handleCreate}>
...
function JoinScreen({ onJoin, onBack }) {
  ...
  async function handleJoin() {
    setLoading(true); setErr("");
    const game = await db.get("games", code.trim().toUpperCase());
    if (!game) { setErr("Code introuvable. Vérifie avec ton pote !"); setLoading(false); return; }
    await db.update("games", game.id, { guest_name: name.trim() });
    setLoading(false);
    onJoin({ ...game, guest_name: name.trim(), myRole: "guest", myName: name.trim() });
  }
  ...
  return (
    <div className="screen">
      ...
      <div className="card">
        ...
        <div className="lbl">Code de la partie</div>
        <input className="inp" placeholder="AB12C" value={code} onChange={e=>setCode(e.target.value.toUpperCase())} style={{fontSize:24,letterSpacing:6,textAlign:"center"}} />
        <button className="btn btn-gold" disabled={!code.trim()||!name.trim()||loading} onClick={handleJoin}>
...
function WaitingScreen({ game, myName, onStart, onBack }) {
  ...
  useEffect(() => {
    if (guestJoined) return;
    const unsub = db.subscribe("games", game.id, (g) => {
  ...
  return (
    <div className="screen">
      ...
      <div className="card">
        <div className="card-title">En attente…</div>
        <p style={{fontSize:13,color:"var(--muted2)",marginBottom:16,lineHeight:1.6}}>Envoie ce code à ton partenaire pour qu'il rejoigne.</p>
        <div className="code-box">
          <div className="code-big">{game.id}</div>
          <div className="code-hint">Code de la partie</div>
        </div>
        <div className="status-row">
          <div className="pill ok">✓ {myName}</div>
          <div className={`pill ${guestJoined?"ok":""}`}>{guestJoined?"✓ Connecté(e)":"⏳ En attente…"}</div>
...
function QuestionScreen({ game, myRole, myName, idx, total, onAnswer }) {
  const [answer, setAnswer] = useState("");
  const q = game.questions[idx];
  async function submit() {
    if (!answer.trim()) return;
    ...
    await db.update("games", game.id, patch);
    onAnswer(answer.trim());
  }
  return (
    <div className="q-screen">
      ...
      <div className="q-footer">
        <textarea key={idx} className="q-textarea" placeholder="Ta réponse…" value={answer} onChange={e=>setAnswer(e.target.value)} autoFocus/>
        <button className="btn btn-gold" disabled={!answer.trim()} onClick={submit}>
...
function DilemmeScreen({ game, myRole, idx, total, onAnswer }) {
  ...
  return (
    <div className="d-screen">
      ...
      <div className="d-footer">
        <button className="btn btn-gold" disabled={!chosen} onClick={submit}>
          {idx+1===total?"Terminer ✓":"Suivant →"}
        </button>
      </div>
    </div>
...
function WaitingRevealScreen({ game, myName, myRole, onReveal }) {
  const [liveGame, setLiveGame] = useState(game);
  const otherName = myRole === "host" ? (game.guest_name || "Ton partenaire") : game.host_name;
  const myDone = myRole === "host" ? liveGame.host_done : liveGame.guest_done;
  const otherDone = myRole === "host" ? liveGame.guest_done : liveGame.host_done;

  useEffect(() => {
    const unsub = db.subscribe("games", game.id, setLiveGame);
    return unsub;
  }, []);

  useEffect(() => {
    if (liveGame.reveal_started) onReveal(liveGame);
  }, [liveGame.reveal_started]);

  async function launchReveal() {
    await db.update("games", game.id, { reveal_started: true, reveal_index: 0 });
  }
...
function RevealScreen({ game, myRole, myName, revealIdx, onNext, isLast }) {
  ...
  async function handleNext() {
    if (!isLast) await db.update("games", game.id, { reveal_index: revealIdx + 1 });
    onNext();
  }
  ...
  return (
    <div className="rv-screen">
      ...
      <div className="rv-question-box">
        <div className="rv-question-text">{game.questions[revealIdx]}</div>
      </div>
      <div className="rv-answers">
        <div className="rv-answer-card">
          <div className="rv-answer-name">👤 {myName}</div>
          <div className="rv-answer-text">{myAnswers?.[revealIdx] || "…"}</div>
        </div>
        <div className="rv-answer-card">
          <div className="rv-answer-name">👤 {otherName}</div>
          <div className="rv-answer-text">{theirAnswers?.[revealIdx] || "…"}</div>
        </div>
      </div>
      <div className="next-btn-wrap">
        <button className="btn btn-gold" onClick={handleNext}>{isLast?"🎉 Terminer":"Question suivante →"}</button>
...
function DilemmeRevealScreen({ game, myRole, myName, revealIdx, onNext, isLast }) {
  ...
  return (
    <div className="rv-screen">
      ...
      <div className="rv-question-box">
        <div className="rv-question-text">T'aurais plutôt… {d.a} <em>ou</em> {d.b} ?</div>
      </div>
      <div style={{padding:"12px 16px",borderBottom:"1px solid var(--border)",flexShrink:0}}>
        <div className={`dr-match ${match?"yes":"no"}`}>
          {match ? "✓ Vous avez choisi pareil !" : "✗ Vous êtes pas d'accord — débattez !"}
        </div>
        <div className="rv-answers" style={{padding:0,border:"none"}}>
          <div className="rv-answer-card">
            <div className="rv-answer-name">👤 {myName}</div>
            <div className="rv-answer-text">{myC === "a" ? d.a : myC === "b" ? d.b : "…"}</div>
          </div>
          <div className="rv-answer-card">
            <div className="rv-answer-name">👤 {otherName}</div>
            <div className="rv-answer-text">{theirC === "a" ? d.a : theirC === "b" ? d.b : "…"}</div>
          </div>
        </div>
      </div>
      <div className="next-btn-wrap">
...
function EndScreen({ theirName, onRestart }) {
  return (
    <div className="end-screen">
      ...
      <div className="wordmark" style={{fontSize:38}}>Reveal</div>
      <p style={{color:"var(--muted2)",fontSize:15,lineHeight:1.7,margin:"18px 0 32px",maxWidth:280}}>
        Partie terminée avec <strong style={{color:"var(--text)"}}>{theirName}</strong>.<br/>
        J'espère que vous vous connaissez un peu mieux. 💛
      </p>
      <button className="btn btn-gold" style={{maxWidth:300,width:"100%"}} onClick={onRestart}>🔄 Nouvelle partie</button>
    </div>
  );
...
export default function App() {
  ...
  return (
    <>
      ...
      {screen === "create" && (
        <CreateScreen
          onStart={g => { setGame(g); setMyRole("host"); setMyName(g.myName); setMyAnswers([]); setCurrentQ(0); setRevealIdx(0); setScreen("waiting"); }}
      ...
      {screen === "join" && (
        <JoinScreen
          onJoin={g => { setGame(g); setMyRole("guest"); setMyName(g.myName); setMyAnswers([]); setCurrentQ(0); setRevealIdx(0); setScreen(isDilemme || g.mode?.startsWith("dilemme") ? "dilemme" : "question"); }}
          onBack={() => setScreen("home")}
        />
      )}

      {screen === "waiting" && game && (
        <WaitingScreen game={game} myName={myName} myRole={myRole}
          onStart={() => setScreen(isDilemme ? "dilemme" : "question")}
          onBack={() => setScreen("home")} />
      )}

      {screen === "question" && game && (
        <QuestionScreen key={currentQ} game={game} myRole={myRole} myName={myName}
          idx={currentQ} total={total} onAnswer={handleAnswer} />
      )}

      {screen === "dilemme" && game && (
        <DilemmeScreen key={currentQ} game={game} myRole={myRole}
          idx={currentQ} total={total} onAnswer={handleAnswer} />
      )}

      {screen === "waitingReveal" && game && (
        <WaitingRevealScreen game={game} myName={myName} myRole={myRole}
          onReveal={g => { if (g) setGame(g); setRevealIdx(0); setScreen(isDilemme ? "revealDilemme" : "reveal"); }} />
      )}

      {screen === "reveal" && game && (
        <RevealScreen game={game} myRole={myRole} myName={myName}
          revealIdx={revealIdx} onNext={handleNext} isLast={revealIdx + 1 === total} />
      )}

      {screen === "revealDilemme" && game && (
        <DilemmeRevealScreen game={game} myRole={myRole} myName={myName}
          revealIdx={revealIdx} onNext={handleNext} isLast={revealIdx + 1 === total} />
      )}

      {screen === "end" && (
        <EndScreen theirName={otherName} onRestart={() => { setGame(null); setMyRole(null); setMyName(""); setMyAnswers([]); setCurrentQ(0); setRevealIdx(0); setScreen("home"); }} />
      )}
    </>
  );
}
